...
\begin{definition}
	...
	$y_0 = (q_0, p_0) \in \mathbb{R}^{2d}$ the initial value at $t_0 \in \mathbb{R}$. 
...
\begin{definition}\label{def_la_sympnet}
	(LA-SympNet)
	\todo{linear and activation layers}
\end{definition}

\begin{definition}\label{def_g_sympnet}
	(G-SympNet)
	\todo{gradient layers only}
\end{definition}
...
The result Hamiltonian system has a large phase space dimension $d=n \gg 1$.
...
we implicitly assume that the PDE itself does not depend on the position variable $x$.
We alsu assume that the grid points are equidistant. Both conditions are met
...
starting from $y_0$, i.e. $x_i = y_{i-1} = \phi^{\text{h}}_{t,H}(x_i)$ for
$i = 1, \dots, n_{\text{train}} + n_{\text{test}}$. The data is generated
for the fixed time $t = 0.01$ in all experiments. 
The first $n_{\text{train}}$ go to the training data, the remaining samples
to the test data.
The training size
...
\todo{Transport problem, finish this section}
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\end{axis}
	\end{tikzpicture}
	\caption{Predicted displacement $q(t=9,x)$ for the
	linear wave equation experiment.}
\end{figure}

\todo{The choice of basis for the symmetric convolution kernels 
has a large influence on the test loss!}
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\end{groupplot}
	\end{tikzpicture}
	\caption{Predicted displacement $q(t,x)$ for $t=3$ and $t=9$ for the
	Sine-Gordon experiment.}
\end{figure}
