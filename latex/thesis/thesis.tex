...
\todo{We work with small training data. Thus, we do not split the training data into mini-batches.}

\todo{Initialization: Biases are initialized with $0$, all other parameters are initialized randomly from
normal distribution with mean $0$ and standard deviation $0.01$.}

\todo{Adam optimizer, AMSgrad variant}

\subsection{Low-dimensional systems}

\todo{Low dimensional systems with $d = 1$. Flow with fixed time $t = 0.1$}

In this section, the training data is always uniformly sampled from a compact set in phase space.
Let $\mathcal{D} \subset R^2$ denote a compact set. We uniformly choose $n_{\text{train}} \in \N$ samples
$x_1, \dots x_{\text{train}} \in \mathcal{D}$.
Based on these samples, we generate the training data $\mathcal{T}$ 
with the $q$-staggered Störmer-Verlet integrator $\phi^{\text{h}}_{t,H}$
for fixed time $t = 0.01$ and the respective Hamiltonian $H$:
\begin{equation*}
	\mathcal{T} = \{ (x_i, \phi^{\text{h}}_{t,H}(x_i)) \}_{i=1}^{n_{\text{train}}}
\end{equation*}
In addition to the training data $\mathcal{T}$, we generate test data $\mathcal{V}$
to compute the test loss. The test data is generated with the same procedure as the training
data $\mathcal{T}$. We denote the number of samples in $\mathcal{V}$ with $n_{\text{test}} \in \N$.

\todo{Describe different architectures, hyperparameters.}
...
The flow for the Harmonic Oscillator is linear. \todo{Show why flow is linear}
...
\begin{tikzpicture}
	...
	\end{axis}
\end{tikzpicture}
\caption{Test loss for Harmonic Oscillator ($500$ epochs). The linear SympNet consists out of nine alternating upper and 
lower linear layers. The training data is generated from $n_{\text{train}}=40$ samples from $\mathcal{D} = [-2,2] \times [-2,2]$.
The test data is generated from $n_{\text{test}}=400$ samples from $\mathcal{D} = [-2,2] \times [-2,2]$.
...
\end{equation*}
For the experiment, we choose $m=1, g=1$ and $l=1$. We conduct two different experiments with different
training and test data originating from different $\mathcal{D}$:
\begin{align*}
...
\end{align*}
We sample $n_{\text{train}} = 40$ training samples and $n_{\text{test}} = 400$ test samples in all 
simple pendulum experiments. The test data is sampled from the same compact set as the training data.

\cref{fig_pend_exp1_loss} and \cref{{table_pend_exp1_loss}} show the training and test loss for the different architectures
and activation functions with training and test data from $\mathcal{D}_{\text{swing}}$ and learning rate $\gamma = 0.01$.
Note that incorporating normalization results in better training and test losses, and may also stabilize
the learning process. The SympNets do not tend to overfit, in contrast to the fully-connected network (FNN).
...
\begin{tikzpicture}
	...
	\end{groupplot}
\end{tikzpicture}
\caption{Training and test loss for simple pendulum with training and test data from $\mathcal{D}_{\text{swing}}$.
...
\begin{table}
	...
	\caption{Test losses for simple pendulum after $10^5$ epochs. 
	Training and test data sampled from $\mathcal{D}_{\text{swing}}$.}\label{table_pend_exp1_loss}
\end{table}
...
\begin{figure}
	...
	\caption{Phase plots for simple pendulum. Training and test data uniformly sampled from $\mathcal{D}_{\text{swing}}$.
...
\begin{figure}
	...
	Training and test data uniformly sampled from $\mathcal{D}_{\text{swing}}$.
...
is sampled from $\mathcal{D}_{\text{swing+rotate}}$ instead.
\todo{we have also increased the number of layers --> describe hyperparameters!}
We perform the experiment with learning rate $\gamma = 0.01$ and faster learning rate $\gamma = 1.0$.
\cref{table_pend_exp2_loss} shows the final test loss for $\gamma = 0.01$.
\cref{table_pend_exp2_loss_fast_lr} shows the final test loss for $\gamma = 1.0$.
In this case, normalization does not lead to an improvement of prediction performance.
However, normalization allows for faster learning rates.
Still, the faster learning rate does not lead to a better test loss
after the same amount of epochs in this case.
The LA-SympNets, with and without normalization, fail to learn the flow.
...
in the rotating case. This circumstance makes learning the neural network hard for the rotating case, especially if the
goal is to have a good prediction over a long time. This issue needs further investigation.
See \cref{sec_outlook} for ideas how this issue could be tackled.

In general, the first variants of the normalization layers (N1) before better or equal
to the second variants of the normalization layers (N2). The only case where the N2 normalization
layers perform better is with training data from $\mathcal{D}_{\text{swing+rotate}}$ 
and tanh activation, see \cref{fig_pend_exp2_loss}.
...
\begin{tikzpicture}
	\begin{groupplot}[
		...
		\nextgroupplot[
			...
			\addplot[
				color=blue
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot/large-g-sympnet/sigmoid/test_loss.csv};
	
			\addplot[
				color=olive,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot/large-n1-g-sympnet/sigmoid/test_loss.csv};
	
			\addplot[
				color=cyan,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot/large-n2-g-sympnet/sigmoid/test_loss.csv};
		...
		\nextgroupplot[title={tanh ($\gamma = 0.01$)}]
			...
			\addplot[
				color=blue
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot/large-g-sympnet/tanh/test_loss.csv};
	
			\addplot[
				color=olive,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot/large-n1-g-sympnet/tanh/test_loss.csv};
	
			\addplot[
				color=cyan,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot/large-n2-g-sympnet/tanh/test_loss.csv};
		...
		\nextgroupplot[title={ELU ($\gamma = 0.01$)}]
			...
			\addplot[
				color=blue
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot/large-g-sympnet/elu/test_loss.csv};
	
			\addplot[
				color=olive,,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot/large-n1-g-sympnet/elu/test_loss.csv};
	
			\addplot[
				color=cyan,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot/large-n2-g-sympnet/elu/test_loss.csv};

		%%% FROM HERE: fast learning rate
		\nextgroupplot[title={Sigmoid ($\gamma = 1.0$)}]
			\addplot[
				color=olive,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot_fast_lr/large-n1-g-sympnet/sigmoid/test_loss.csv};
	
			\addplot[
				color=cyan,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot_fast_lr/large-n2-g-sympnet/sigmoid/test_loss.csv};
	
		\nextgroupplot[title={tanh ($\gamma = 1.0$)}]	
			\addplot[
				color=olive,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot_fast_lr/large-n1-g-sympnet/tanh/test_loss.csv};
	
			\addplot[
				color=cyan,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot_fast_lr/large-n2-g-sympnet/tanh/test_loss.csv};
	
		\nextgroupplot[title={ELU ($\gamma = 1.0$)}]
			\addplot[
				color=olive,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot_fast_lr/large-n1-g-sympnet/elu/test_loss.csv};
	
			\addplot[
				color=cyan,densely dashed
			] table[x=epoch,y=loss,col sep=comma] {../data/simple_pendulum_swing_rot_fast_lr/large-n2-g-sympnet/elu/test_loss.csv};
				
	\end{groupplot}
\end{tikzpicture}
\\[5pt]
\ref{grouplegend}
\caption{Training and test loss for simple pendulum with training and test data from $\mathcal{D}_{\text{swing+rotate}}$.
(First row) Learning rate $\gamma = 0.01$. (Second row) Faster learning rate $\gamma = 1.0$.
Architectures not included in individual plots have either larger losses than $1 \cdot 10^{-2}$ or diverged with NaN.
Note that normalization does not improve prediction performance in this case. However, normalization
allows faster learning rates.}\label{fig_pend_exp2_loss}
\end{figure}
...
\begin{table}
	...
	Training and test data sampled from $\mathcal{D}_{\text{swing+rotate}}$.}
...
\begin{table}
	...
	\caption{Test losses for simple pendulum after $3 \cdot 10^5$ epochs
	with faster learning rate $\gamma = 1.0$.
	Training and test data sampled from $\mathcal{D}_{\text{swing+rotate}}$.
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\nextgroupplot[
			...
			\addplot[
				color=blue
			] table[x=q,y=p,col sep=comma] {../data/simple_pendulum_swing_rot/large-g-sympnet/tanh/swinging_case/phase_plot.csv};
		...
		\nextgroupplot[title={Phase plot for $q_0 = \pi,\, p_0=1$}]
			...
			\addplot[
				color=blue
			] table[x=q,y=p,col sep=comma] {../data/simple_pendulum_swing_rot/large-g-sympnet/tanh/rotating_case/phase_plot.csv};
...
\begin{figure}
	...
	\begin{tikzpicture}
		...
		\nextgroupplot[title={Total energy $H$ for $q_0 = \frac{\pi}{2},\, p_0 = 0$}, ylabel={$H(t)$}, xmin=0, xmax=100]
			...
			\addplot[
				color=blue
			] table[x=t,y=E,col sep=comma] {../data/simple_pendulum_swing_rot/large-g-sympnet/tanh/swinging_case/total_energy.csv};
		...
		\nextgroupplot[title={Predicted position $q$ for $q_0 = \frac{\pi}{2},\, p_0 = 0$}, ylabel={$q(t)$}, xmin=91, xmax=97, legend pos=north west]
			...
			\addplot[
				color=blue
			] table[x=t,y=q,col sep=comma] {../data/simple_pendulum_swing_rot/large-g-sympnet/tanh/swinging_case/q.csv};
		...
		\nextgroupplot[title={Total energy $H$ for $q_0 = \pi,\, p_0=1$}, ylabel={$H(t)$}, xmin=0, xmax=10]
			...
			\addplot[
				color=blue
			] table[x=t,y=E,col sep=comma] {../data/simple_pendulum_swing_rot/large-g-sympnet/tanh/rotating_case/total_energy.csv};
		...
		\nextgroupplot[title={Predicted position $q$ for $q_0 = \pi,\, p_0=1$}, ylabel={$q(t)$}, xmin=8, xmax=9, legend pos=north west]
			...
			\addplot[
				color=blue
			] table[x=t,y=q,col sep=comma] {../data/simple_pendulum_swing_rot/large-g-sympnet/tanh/rotating_case/q.csv};
...
initial values $y_0 = (q_0^T, p_0^T)^T \in \R^{2n}$, $q_0 = \lb u_0(x_i) \rb_{i=1}^n \in \R^n$, 
$p_0 = \lb w_0(x_i) \rb_{i=1}^n \in \R^n$,
...
\begin{tikzpicture}
	\begin{axis}[
		...
		legend entries={
			CNN,
			CNN-SympNet (canonical basis),
			CNN-SympNet (FD basis)
		}
	]
...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			title={$q(t=9,x)$ (Linear wave equation)},
			xlabel={$x$}, xmin=-0.5, xmax=0.5,
			ylabel={$q(t=9,x)$}, ymin=-0.3, ymax=0.1, restrict y to domain=-2:2,
			legend style={
...
nonlinear wave equation with $G(u) = 1 - cos(u)$, $g(u) = sin(u)$ and $c=1$.

initial values
\begin{align*}
	u_0(x) &= 4 arctan \lsb exp \lb \frac{x-x_0}{\sqrt{1-v^2}} \rb \rsb \\
	w_0(x) &= \dd{x} u_0(x)
\end{align*}
boundary conditions $u_{\text{left}} = 0, \, u_{\text{right}} = 2\pi$.
