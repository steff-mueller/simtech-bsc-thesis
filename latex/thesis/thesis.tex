...
proposed by Jin et. al  in \cite{Jin2020} \todo{How to cite with name?}.
...
\begin{corollary}\label{gradient_corollary}
	Let $V: \mathbb{R}^d \to \mathbb{R}, \quad p \mapsto V(p)$ be a function in 
...
\begin{proof}
	...
	$S := A+A^T$ symmetric. Symplecticity follows with Corollary \ref{gradient_corollary}.
\end{proof}
...
\todo{Linear layer = composition of $\ell_{up}$ and $\ell_{low}$.}
...
$d \in \mathbb{N}$ denotes the width of the gradient layer and can be chosen freely.
