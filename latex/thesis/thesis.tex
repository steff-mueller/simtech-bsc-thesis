...
\todo{Low dimensional systems with $d = 1$.}
...
\begin{table}
	...
	low-dimensional experiments. The activation function used inside the activation layers
	and gradient layers is either sigmoid, tanh or ELU and is explicitly stated.
	}\label{table_low_dim_arch}
\end{table}

\todo{Flow with fixed time $t = 0.1$}
...
\begin{table}
	...
	\begin{tabular}{lp{8cm}c}
		...
		& $809$ \\
		%
		CG-SympNet & A SympNet consisting out of four consecutive blocks of
		the following layers: upper (or lower) $1$-dimensional symplectic convolution layer with
		...
		(variant 1) instead of convolution gradient layers. \todo{Add reference}
		& $2008$ \\
		%
		N2-CG-SympNet & Same as CG-SympNet, but with normalized convolution gradient layers
		(variant 2) instead of convolution gradient layers. \todo{Add reference}
		& $2008$ \\ \bottomrule
	\end{tabular}
