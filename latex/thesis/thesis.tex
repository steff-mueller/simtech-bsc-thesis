...
\begin{definition}
	...
	and $0 < \int \abs{\frac{\mathrm{d}^r}{\mathrm{d}x^r} \sigma} d\lambda < \infty$. $\lambda$ 
...
is $r$-finite for $r \in \N$. This implies that the $\mathrm{tanh}$ activation function,
...
the chosen activation function has a greater influence on model performance for
$\mathcal{D}_{\text{swing+rotate}}$.
...
LA-SympNets in the previous case $\mathcal{D}_{\text{swing}}$ does not transfer to this case with
$\mathcal{D}_{\text{swing+rotate}}$. For $\mathcal{D}_{\text{swing+rotate}}$, the SympNets
do not work well with the SympNets, whereas the FNN with ELU activation still achieves 
...
\begin{figure}
	...
	linear wave equation experiment. The C-SympNet with FD basis is indistinguishable
...
because it applies the same kernels on the whole input $(q,p)$. 
...
The extrapolation performance of the two best performing architectures and the LARGE-CNN
...
\todo{better long-time prediction performance, extrapolation}
...
\todo{approximation theorems, non-$r$-finite, periodic}
...
\todo{PyTorch, Python, Docker, Tensorflow, git, GitLab}

\todo{Visual Studio Code, SSH}

\todo{reference ReadMe in code}
