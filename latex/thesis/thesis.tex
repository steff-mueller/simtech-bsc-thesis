...
\todo{Discuss different parametrizations for symmetric kernels}
...
\begin{align*}
	constant\_pad[s_p,l,r] &: \mathbb{R}^{L_{in}} \to \mathbb{R}^{L_{in}+2s_p}, \\
...
\todo{Describe what symmetric padding does in prose}
\begin{align*}
	symmetric\_pad[s_p] &: \mathbb{R}^{L_{in}} \to \mathbb{R}^{L_{in}+2s_p}, \\
...
or $pad[s_p,\dots] = symmetric\_pad[s_p]$. For $pad[s_p,\dots] = symmetric\_pad[s_p]$
it must hold $d \geq m$. $k \in \mathbb{R}^{2m+1}$ is learnable during training. 
$l,r \in \mathbb{R}^{s_p}$ are constant or also learnable during training.
\todo{Make the sentences above less cluttered...}
...
\end{theorem}
\begin{proof}
	Let $i \in \mathbb{N}$ with $ 1 \leq i \leq d$.
	Then
	\begin{equation*}
		\lb conv1d \lsb k \rsb \circ symmetric\_pad \lsb m \rsb \rb_i
		= \sum_{u=-m}^m k_u \lb symmetric\_pad \lsb m \rsb \rb_{i+u+m} = (*)
	\end{equation*}
	With the variable transform $v=v(u):=i+u+m \iff u=v-i-m$ we have
	\begin{equation*}
		(*) = \sum_{v=i}^{i+2m} k_{v-i-m} \lb symmetric\_pad \lsb m \rsb \rb_{v}
	\end{equation*}
	...
	Then
	\begin{align*}
		(*) &= \sum_{v_1 \in I_1} k_{v_1-i-m} x_{m+1-v_1} +
		\sum_{v_2 \in I_2} k_{v_2-i-m} x_{v_2-m} +
		\sum_{v_3 \in I_3} k_{v_3-i-m} x_{2d+m+1-v_3}
	\end{align*}
	...
	Then
	\begin{align*}
		(*) = \sum_{w_1 \in \widetilde{I_1}} k_{1-i-w_1} x_{w_1} +
		\sum_{w_2 \in \widetilde{I_2}} k_{w_2-i} x_{w_2} +
		\sum_{w_3 \in \widetilde{I_3}} k_{2d+1-i-w_3} x_{w_3}
	\end{align*}
	Let $j \in \mathbb{N}$ with $ 1 \leq j \leq d$. Then for the Jacobian holds
	\begin{align*}
		\hspace{6em}&\hspace{-6em}
		\lb conv1d \lsb k \rsb \circ symmetric\_pad \lsb m \rsb \rb_{ij}
		= \deldel{p_j} \lb conv1d \lsb k \rsb \circ symmetric\_pad \lsb m \rsb \rb_i \\
