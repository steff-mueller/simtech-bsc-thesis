...
Additionially, we define a shift operator $\mathcal{S}_k$, 
which shifts a discrete function $f \in \mathbb{R}^\mathbb{Z}$ by $k \in \mathbb{Z}$ positions
in the right direction
\begin{equation*}
	\mathcal{S}_k : \mathbb{R}^{\mathbb{Z}} \to \mathbb{R}^{\mathbb{Z}},
	\quad \lb \mathcal{S}_v(f) \rb (\tau) := f(\tau - k)
\end{equation*}
...
We define a stride operator $\mathcal{D}_k$ for a stride number $k \in \mathbb{N}$ with
\begin{equation*}
	\mathcal{D}_k : \mathbb{R}^{\mathbb{Z}} \to \mathbb{R}^{\mathbb{Z}},
	\quad (\mathcal{D}_k)(\tau) := f(k \tau)
\end{equation*}
The stride operators skips $k$ entries of $f \in \mathbb{R}^\mathbb{Z}$.
...
Valid cross-correlation is a linear operator. Therefore
...
\begin{tikzpicture}
	...
	\addplot[
		color=blue,
		no markers
	] table[x=q,y=p,col sep=comma] {../data/simple_pendulum_swing_rot/la-sympnet/phase_plot.csv};

	\addplot[
		color=green,
		no markers
	] table[x=q,y=p,col sep=comma] {../data/simple_pendulum_swing_rot/g-sympnet/phase_plot.csv};

	\addplot[
		color=blue,
		dashed,
		no markers
	] table[x=q,y=p,col sep=comma] {../data/simple_pendulum_swing_rot/normalized-la-sympnet/phase_plot.csv};

	\addplot[
		color=green,
		dashed,
		no markers
	] table[x=q,y=p,col sep=comma] {../data/simple_pendulum_swing_rot/normalized-g-sympnet/phase_plot.csv};
