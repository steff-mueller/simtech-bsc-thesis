...
1d convolution bc. we only have systems with $1$-dimensional spatial domain.
\todo{Should be extended if we do $3$d system.}
...
\begin{equation*}
	constant\_pad[p,l,r]: \mathbb{R}^{L_{in}} \mapsto \mathbb{R}^{L_{in}+2p},
	(constant\_pad[p,l,r](x))_i :=
	\begin{cases}
		l_i & 1 \leq i \leq p \\
		x_{i-p} & p+1 \leq i \leq L_{in}+p \\
		r_{i-L_{in}+p} & L_{in}+p+1 \leq i \leq L_{in} + 2p
	\end{cases}
\end{equation*}
where $p \in \mathbb{N}$ is the padding size and $l,r \in \mathbb{R}^p$
...
\begin{align*}
	\mathcal{C}_{up} \qpvec &:= \uppersympop{conv1d[k] \circ pad[p,\dots]} \\
	\mathcal{C}_{low} \qpvec &:= \lowersympop{conv1d[k] \circ pad[p,\dots]}
\end{align*}
with kernel $k \in \mathbb{R}^{2m+1}$ symmetric, $p = m$
and either $pad[p,\dots] = constant\_pad[p,l,r]$ ($l,r \in \mathbb{R}^p$) 
or $pad[p,\dots] = reflect\_pad[p]$. $k \in \mathbb{R}^{2m+1}$ is learnable during training. 
$l,r \in \mathbb{R}^p$ can be constant or also learnable during training.

Indeed $conv1d[k] \circ pad[p,\dots] : \mathbb{R}^d \to \mathbb{R}^d$ because
\begin{align*}
	L_{in,pad[p,\dots]} &= d \\
	L_{out,pad[p,\dots]} &= L_{in,pad[p,\dots]} + 2p = d+2p = d+2m \\
...
\begin{align}
	L_{in,conv1d[k]} &= L_{out,pad[p,\dots]} = d+2m \label{conv_dim1} \\ 
	L_{out,conv1d[k]} &= L_{in,conv1d[k]} - s_k + 1 = d \label{conv_dim2}
\end{align}
indeed imply that the kernel size $s_k$ must be odd.
\begin{equation*}
	\text{\cref{conv_dim1} in \cref{conv_dim2}:} \quad d+2m - s_k + 1 = d \iff s_k = 2m+1
\end{equation*}
