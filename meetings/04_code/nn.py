...
from symplecticloss import symplectic_loss

from linearsymplectic import LinearSymplectic
...
def train_fnn(X_train, Y_train, epochs=1000, liveloss=True, device=None):
    ...
    mse_loss = lambda y_target, model, x, dev: mse(model(x), y_target)

    return train(nn_model, mse_loss, X_train, Y_train, epochs, liveloss, device)

def train_softfnn(X_train, Y_train, epochs=1000, liveloss=True, device=None):
    nn_model = torch.nn.Sequential(
        torch.nn.Linear(2, 50),
        torch.nn.Sigmoid(),
        torch.nn.Linear(50, 50),
    ...
    return train(nn_model, symplectic_loss, X_train, Y_train, epochs, liveloss, device)

def train_sympnet(X_train, Y_train, epochs=1000, liveloss=True, device=None):
    sub_layers = 8
    d = 1
    nn_model = torch.nn.Sequential(
        LinearSymplectic(sub_layers, d, dt),
        LowerNonlinearSymplectic(d, dt, bias=False),
        LinearSymplectic(sub_layers, d, dt),
        UpperNonlinearSymplectic(d, dt, bias=False),
        LinearSymplectic(sub_layers, d, dt),
        LowerNonlinearSymplectic(d, dt, bias=False),
        LinearSymplectic(sub_layers, d, dt),
        UpperNonlinearSymplectic(d, dt, bias=False),
...
def train(nn_model, loss_fn, X_train, Y_train, epochs=1000, liveloss=True, device=None):
    learning_rate = 1e-1
    ...
    for t in range(epochs):
        loss = loss_fn(Y_train, nn_model, X_train, device)
